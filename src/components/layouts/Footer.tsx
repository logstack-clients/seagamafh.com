					...
					<div className="lg:col-span-2 flex flex-col gap-4">
						...
						<ul className="flex flex-col gap-2.5">
							{navigations.map((item) => (
								<li key={item.name}>
									<a href={item.href} className="text-white/70 hover:text-white hover:translate-x-1 flex items-center gap-1.5 text-sm transition-all duration-200">
							...
							<li>
								<a href="/careers" className="text-white/70 hover:text-white hover:translate-x-1 flex items-center gap-1.5 text-sm transition-all duration-200">
									<LuChevronRight className="w-3.5 h-3.5 opacity-60 shrink-0" />
						...
						<ul className="flex flex-col gap-2.5">
							{services.items.map((service) => (
								<li key={service.id} className="group">
									<a href="/#services" className="text-white/70 hover:text-white flex flex-col transition-all duration-200">
				...
				<div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-white/50">
					...
					<div className="flex flex-wrap justify-center gap-5 sm:gap-6 font-medium">
						{footer.links.map((link, idx) => (
							<a key={idx} href={link.href} className="hover:text-brand-tertiary border-b border-transparent hover:border-brand-tertiary transition-all duration-200 py-0.5">
								{link.text}
							</a>
						))}
					</div>
