...
import { navigations } from '@/config/sections.config';
		...
		<nav className="fixed top-0 left-0 right-0 z-50 bg-canvas-page/90 backdrop-blur-md shadow-soft py-3 lg:py-4 transition-all duration-300">
			<div className="max-w-336 mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between gap-4">
				...
				<div className="hidden md:flex items-center gap-1 lg:gap-6 xl:gap-8 shrink min-w-0 justify-end flex-1">
						...
						{navigations.map((link) => {
							...
							return (
								<a
									key={link.name}
									href={link.href}
									className={`text-[13px] lg:text-sm font-bold transition-all px-2.5 lg:px-3 py-1.5 rounded-full whitespace-nowrap ${isActive ? 'text-brand-primary bg-brand-primary/10' : 'text-text-muted hover:text-brand-primary hover:bg-canvas-alt'}`}>
									{link.name}
								</a>
							);
						})}
					...
					<a
						href="/#contact"
						className="bg-btn-cta text-white px-4 lg:px-6 py-2 lg:py-2.5 rounded-full text-[13px] lg:text-sm font-semibold whitespace-nowrap shrink-0 hover:bg-btn-cta-hover transition-all transform hover:scale-105 active:scale-95 shadow-md">
						Book a Tour
					</a>
				</div>
							...
							{navigations.map((link) => {
								...
								return (
									<a
										key={link.name}
										href={link.href}
										className={`text-base font-medium px-4 py-3 rounded-lg transition-colors flex items-center justify-between
										${isActive ? 'bg-brand-primary/5 text-brand-primary' : 'text-text-main hover:bg-canvas-alt'}`}
